...
export const ProjectCard = ({ project }: { project: Project }) => {
  ...
  return (
    <>
      <div className="group relative overflow-hidden cursor-pointer" onClick={handleViewProject}>
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500">
              <div className="absolute bottom-4 left-4 right-4 flex justify-center">
                <div className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 group/btn">
                  ...
                  <span className="font-medium">{project.modalImage ? "View Results" : "View Project"}</span>
                </div>
              </div>
            </div>
            <div className="absolute top-4 right-4">
              <span className={`px-3 py-1 text-xs font-bold rounded-full ${project.status === "Live" ? "bg-green-500/20 text-green-300 border border-green-500/50" : "bg-yellow-500/20 text-yellow-300 border border-yellow-500/50"}`}>
                {project.status}
              </span>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {project.tech.map((t, idx) => (
                <span key={t} className="text-xs px-3 py-1.5 bg-gray-800/80 text-gray-200 rounded-lg border border-gray-700/50 font-medium hover:bg-gray-700/80 hover:border-gray-600/50 transition-all duration-300 transform hover:scale-105" style={{ transitionDelay: `${idx * 50}ms` }}>
                  {t}
                </span>
              ))}
            </div>
            <div className="pt-4 border-t border-gray-700/50">
              ...
              >
                <svg className="w-5 h-5 group-hover/link:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
                <span>{project.modalImage ? "View Results" : "View Project"}</span>
              </button>
            </div>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="relative max-w-4xl w-full bg-gray-900 rounded-2xl overflow-hidden border border-gray-700 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              <h3 className="text-white font-bold text-lg">Digital Marketing Campaign Results</h3>
            ...
            <img src="/assets/images/google-ads-results.png" alt="Campaign Results" className="w-full h-auto" />
            <div className="p-4 grid grid-cols-4 gap-4 border-t border-gray-700">
              <div className="text-center"><p className="text-purple-400 text-xl font-bold">7.6M+</p><p className="text-gray-400 text-sm">Impressions</p></div>
              <div className="text-center"><p className="text-purple-400 text-xl font-bold">287K</p><p className="text-gray-400 text-sm">Clicks</p></div>
              <div className="text-center"><p className="text-purple-400 text-xl font-bold">17,578</p><p className="text-gray-400 text-sm">Conversions</p></div>
              <div className="text-center"><p className="text-purple-400 text-xl font-bold">₹7 Cr</p><p className="text-gray-400 text-sm">Revenue</p></div>
            </div>
          </div>
        </div>
      )}
